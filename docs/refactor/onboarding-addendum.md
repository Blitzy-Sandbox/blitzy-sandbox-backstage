...
The OpenTelemetry Prometheus exporter binds to `127.0.0.1:9464/metrics` **by default** (see `packages/backend/src/instrumentation.js` and decision-log entry 15). Only the metric scraper (Prometheus server, Grafana Cloud agent, Datadog agent, etc.) needs to reach this port, and the default loopback bind keeps it unreachable from external network neighbors. Verify with `ss -tlnp | grep 9464` or `netstat -tlnp | grep 9464` on the running host — the bind address should be `127.0.0.1:9464`, never `0.0.0.0:9464` unless `PROMETHEUS_BIND_HOST` has been intentionally set. If a deployment requires a different bind (host-network Prometheus, multi-NIC host, etc.), set `PROMETHEUS_BIND_HOST` on the backend process to `0.0.0.0` (only safe behind a firewall or NetworkPolicy) or to a specific private interface IP. The exporter emits histogram buckets that label by `email_domain` (`blitzy.com`, `other`, `guest`) and `action` — these labels are PII-safe (no email addresses), but exposure of the volume itself reveals the user population size to anyone who can scrape the endpoint, and the `target_info` metric carries host fingerprint and process ownership data; both are reasons to keep the bind as restrictive as the deployment topology allows.
