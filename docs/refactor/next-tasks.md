...
- **Description:** _Historical context — completed in this pull request._ The `plugins/catalog-backend-module-access-audit/` plugin was created in this refactor to emit `entity-access` audit events whenever a user-credentialed entity read passes through the catalog backend. The plugin's behavior is now covered by the plugin-local unit-test file `plugins/catalog-backend-module-access-audit/src/module.test.ts` (20 executed cases) in addition to the E2E test (`packages/app/e2e-tests/auditing.test.ts`). The unit test covers by-name and by-uid success paths, non-GET exclusion, collection and ancestry exclusion, success and fail finalization, principal variations, auditor failure degradation, finish+close idempotency, canonicalization, and response-body fallback. This entry is retained for historical context per the R3 explainability rule; new follow-on work for the access-audit module should be filed as a fresh entry.
- **Priority:** `completed`
