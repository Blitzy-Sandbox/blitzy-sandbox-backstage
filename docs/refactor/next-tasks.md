...
- `medium` — improves the platform but is not blocking. Either it advances an in-flight migration (Entry 2), adds observability depth (Entry 4), or hardens CI infrastructure (Entry 7).
- `low` — net-new feature work or hygiene with a limited blast radius. Entries 3, 6, 9, and 10 fall here.
- `completed` — retained for historical context per R3. Entry 1 (custom Prometheus counters) and Entry 8 (access-audit middleware unit test) landed in this PR.
...
## 1. Wire the custom Prometheus counters into their planned emission sites — COMPLETED IN THIS PR

- **Description:** _Historical context — completed in this pull request._ The refactor's observability story (per Rule R1) defines three custom Prometheus counters — `user_login_total{provider, email_domain}`, `entity_access_total{action}`, and `blitzy_permission_decisions_total{result, email_domain, action}` — as the operator's quantitative view of sign-in volume, project-access volume, and policy decisions. These counters are now registered through a shared metrics helper module and incremented at their canonical emission sites: `packages/backend/src/authModuleGithubProvider.ts` (success and failure paths), `plugins/catalog-backend-module-access-audit/src/module.ts` (per `entity-access` event), and `plugins/permission-backend-module-blitzy-policy/src/policy.ts` (per ALLOW/DENY decision). The Grafana dashboard template at [`../observability/dashboard-template.json`](../observability/dashboard-template.json) now renders data on the corresponding panels. This entry is retained for historical context per the R3 explainability rule.
- **Priority:** `completed`
- **Suggested approach:** No further action — see [`../observability/dashboards.md`](../observability/dashboards.md) §4.3 for the committed metric catalog and label-value vocabulary.
...
## 8. Add the access-audit middleware plugin-local unit test — COMPLETED IN THIS PR

- **Description:** _Historical context — completed in this pull request._ The `plugins/catalog-backend-module-access-audit/` plugin was created in this refactor to emit `entity-access` audit events whenever a user-credentialed entity read passes through the catalog backend. The plugin's behavior is now covered by the plugin-local unit-test file `plugins/catalog-backend-module-access-audit/src/module.test.ts` (25 executed cases) in addition to the E2E test (`packages/app/e2e-tests/auditing.test.ts`). The unit test covers by-name and by-uid success paths, non-GET exclusion, collection and ancestry exclusion, success and fail finalization, principal variations, auditor failure degradation, finish+close idempotency, canonicalization, and response-body fallback. This entry is retained for historical context per the R3 explainability rule; new follow-on work for the access-audit module should be filed as a fresh entry.
- **Priority:** `completed`
- **Suggested approach:** No further action — see [`traceability-matrix.md`](traceability-matrix.md) Forward Matrix row 9 and Reverse Matrix Sub-group F for the committed cross-references.
...
## 10. Cross-browser visual regression baselines for firefox and webkit

- **Description:** [`decision-log.md`](decision-log.md) Entry 18 documents the cross-browser Playwright fan-out introduced by this PR: every functional test (welcome page, theme correctness, sidebar/View/Documentation/star/System/Owner removals, top-bar placement, Support email, library border, catalog count, Guest sign-in audit events, BlitzyPermissionPolicy read-only enforcement, landing redirect, search behavior — approximately 38 of the 52 tests in `packages/app/e2e-tests/`) now runs on chromium, firefox, and webkit (156 total invocations). Visual regression tests (the 14 cases that consume PNG baselines under `packages/app/e2e-tests/__screenshots__/`) are gated to chromium-only via a `test.beforeEach` title-prefix hook in `app.test.ts` and `SearchPage.test.ts`, because the 10 existing baselines were captured on chromium and cross-browser pixel drift (font hinting, scrollbar widths, sub-pixel anti-aliasing) would produce spurious failures on firefox and webkit. Capturing per-browser baselines closes the gap so AAP §0.8.2.5's cross-browser commitment covers visual fidelity in addition to functional behavior.
- **Priority:** `low`
- **Suggested approach:** (1) Extend `playwright.config.ts`'s `snapshotPathTemplate` to include `{projectName}`, e.g. `'{testDir}/__screenshots__/{projectName}/{testFilePath}/{arg}{ext}'`, so per-browser baselines do not collide. (2) Migrate the existing 10 chromium PNGs from `packages/app/e2e-tests/__screenshots__/app.test.ts/` and the 4 from `SearchPage.test.ts` into `…/example-app-chromium/app.test.ts/` and `…/example-app-chromium/SearchPage.test.ts/` respectively. (3) Remove the chromium-only gate from the `test.beforeEach` hooks (or invert it to "skip if no baseline exists for `${projectName}`"). (4) Run `npx playwright test --project=example-app-firefox --update-snapshots` and `--project=example-app-webkit --update-snapshots` to seed the firefox and webkit baselines from the current rendering. (5) Inspect each generated baseline visually for cross-browser correctness; tune `expect.toHaveScreenshot.maxDiffPixels` (currently 100) and `threshold` (currently 0.2) per-engine if needed by passing per-project `expect` overrides via the `projects` array. (6) Commit the new baselines and document the per-engine tolerance in [`decision-log.md`](decision-log.md) as a follow-up entry; reference Entry 18 as the prior decision being extended. Coordinate with Entry 7 (LocalGCP in CI) so the per-browser run does not collide with the LocalGCP wiring.
