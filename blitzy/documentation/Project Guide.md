...
| Custom Prometheus counters                                       | Planned, NOT YET IMPLEMENTED    | `blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total` are documented as the contract for future work but no registration / increment code exists in `policy.ts`, `authModuleGithubProvider.ts`, or the access-audit `module.ts`. `@opentelemetry/api-metrics` is not installed. Wiring is tracked as [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1. The audit-event channel is the operator's authoritative source of truth for sign-in / entity-access / permission-decision signals at this checkpoint. See [`Technical Specifications.md`](./Technical%20Specifications.md) IR-5. |
...
| `plugins/catalog-backend-module-access-audit/src/module.test.ts` | NOT CREATED at this checkpoint  | Functional coverage for the audit middleware is achieved via `packages/app/e2e-tests/auditing.test.ts`. The dedicated unit test file is tracked as [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 8.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- **Auditors and operators** — receive a complete `user-login` plus `entity-access` event trail in the backend's stdout structured JSON via `coreServices.auditor`. The auto-instrumented OpenTelemetry Prometheus counters (`http_server_request_duration_seconds`, `http_server_requests_total`, `process_resident_memory_bytes`, etc.) are exposed at `http://localhost:9464/metrics` for dashboarding and alerting today. The custom counters originally specified for the audit channel (`user_login_total`, `entity_access_total`, `blitzy_permission_decisions_total`) are documented as the contract for future work but are NOT YET EMITTED at this checkpoint — wiring them is tracked as [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1; the audit-event channel is the operator's authoritative source of truth for sign-in / entity-access volume in the meantime.
...
| Workstream C — Authorization, Audit, User Tracking             | Implemented (with caveats)                     | `BlitzyPermissionPolicy` registered; `user-login` + `entity-access` events emitted via `coreServices.auditor`; unit coverage ≥80% on `BlitzyPermissionPolicy.handle()` and the augmented GitHub `signInResolver`. The dedicated unit test for the access-audit module (`plugins/catalog-backend-module-access-audit/src/module.test.ts`) is NOT YET CREATED at this checkpoint; functional coverage is achieved via the new `auditing.test.ts` E2E suite. Email propagation uses a custom JWT `email` claim (see §0 row "Email propagation to policy"); sign-in audit correlation is synthetic (see §0 row "Sign-in audit correlation"). Custom Prometheus counters are documented but NOT YET EMITTED (see §0 row "Custom Prometheus counters"). |
...
"Implemented" in this table means: the source code for the workstream lives in the repository and is exercised by unit and/or E2E tests. The audit-middleware dedicated unit test, the planned Prometheus counters, and the CI LocalGCP provisioning step are each tracked separately in [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md).
...
- **Medium priority** — Wire the planned custom Prometheus counters (`blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`) into their planned emission sites ([`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1) so the two `dashboard-template.json` panels currently rendering "No data" become functional and so operators have a near-real-time signal alongside the audit-event channel.
...
| `plugins/catalog-backend-module-access-audit/src/module.test.ts`     | ≥80% lines and branches | NOT YET CREATED   | NOT YET CREATED at this checkpoint — functional coverage is achieved via `packages/app/e2e-tests/auditing.test.ts`; the dedicated unit test is tracked as [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 8 |
...
A dedicated audit-middleware unit test (`plugins/catalog-backend-module-access-audit/src/module.test.ts`) is planned but NOT YET CREATED at this checkpoint. Functional coverage that the `entity-access` event is emitted on user-credentialed entity reads (carrying the canonical HTTP correlation id via `request: req`) lives in the new `packages/app/e2e-tests/auditing.test.ts` Playwright suite. The dedicated unit suite — covering anonymous service-to-service reads (no audit event emitted), reads for nonexistent entities (still audited because the request reached the catalog service), and reads with a malformed entity ref (audited with the ref as-provided) — is tracked in [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 8.
...
Available today (auto-instrumented metrics emitted by `getNodeAutoInstrumentations()`):

- `curl http://localhost:9464/metrics | grep '^http_server_request_duration_seconds_bucket'` returns the HTTP request latency histogram (auto-instrumented; available today).
- `curl http://localhost:9464/metrics | grep '^http_server_requests_total'` returns the HTTP request counter by status code (auto-instrumented; available today).
- `curl http://localhost:9464/metrics | grep '^process_resident_memory_bytes'` returns process memory metrics (auto-instrumented; available today).
- `curl http://localhost:9464/metrics | grep '^nodejs_heap_size_used_bytes'` returns Node.js heap usage (auto-instrumented; available today).
...
PLANNED (NOT YET EMITTED at this checkpoint — see §0 row "Custom Prometheus counters" and [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1):

- ~~`curl http://localhost:9464/metrics | grep blitzy_permission_decisions_total`~~ — will return no rows until the policy counter is wired into `BlitzyPermissionPolicy.handle()`.
- ~~`curl http://localhost:9464/metrics | grep user_login_total`~~ — will return no rows until the sign-in counter is wired into the augmented `signInResolver`.
- ~~`curl http://localhost:9464/metrics | grep entity_access_total`~~ — will return no rows until the entity-access counter is wired into the audit middleware.

Until the custom counters land, the audit-event channel (`grep '"eventId":"user-login"'` or `grep '"eventId":"entity-access"'` against the backend's stdout structured log) is the operator's authoritative source of truth for the corresponding signals.

The Grafana dashboard imported from [`docs/observability/dashboard-template.json`](../../docs/observability/dashboard-template.json) renders six panels on import. Four panels (Catalog Query Latency, HTTP Error Rate, Node.js Heap, Service Health) are functional today against the auto-instrumented metrics; two panels (Audit Events Per Minute, Permission Decisions) display "No data" until the planned custom counters land.
...
- **Workstream C — Authorization and audit verification**:
  ...
  Expected: the refresh attempt returns `403` for the Guest token; the structured log shows `user-login` (with a synthetic `correlationId`) and `entity-access` (with the canonical HTTP correlation id from `request: req`) events. The `grep` against the custom Prometheus counters (`user_login_total`, `entity_access_total`, `blitzy_permission_decisions_total`) is DEFERRED — those counters are NOT YET EMITTED at this checkpoint; see §0 row "Custom Prometheus counters" and [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1. When the counters land, the original `curl … | grep -E '^(user_login_total|entity_access_total|blitzy_permission_decisions_total)'` command becomes the canonical verification.
...
- **Observability verification (Rule R1)**:
  ...
  Expected: the metrics endpoint returns Prometheus-format counters (auto-instrumented HTTP / process / Node.js metrics today; planned custom counters when [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1 lands); the liveness and readiness probes return `200 OK` with bodies indicating service status. NOTE: the canonical Backstage 1.48.0 health paths are `/.backstage/health/v1/{liveness,readiness}` via `coreServices.rootHealth`; the previously-documented `/health` and `/readiness` aliases are NOT wired in this repository (see §0 row "Backend health/readiness endpoints").
...
| ---- | ------------------------------------------------------------------------------------------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1   | Observability — structured logging, distributed tracing, metrics endpoint, health and readiness, dashboard template | PARTIAL    | OpenTelemetry SDK wired in `packages/backend/src/instrumentation.js`; Prometheus exporter on `:9464/metrics` emits auto-instrumented metrics (HTTP, process, Node runtime); canonical health endpoints at `/.backstage/health/v1/{liveness,readiness}` via `coreServices.rootHealth`; new Grafana template at [`docs/observability/dashboard-template.json`](../../docs/observability/dashboard-template.json) with 4 of 6 panels functional today and 2 panels pending custom counters; new docs at [`docs/observability/dashboards.md`](../../docs/observability/dashboards.md). Gaps: the three custom counters (`blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`) are documented but NOT YET EMITTED at this checkpoint — see §0 row "Custom Prometheus counters" and [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1. |
...
**Success Metrics.** Unit test coverage of new and modified authentication / authorization logic is ≥80% per AAP §0.8.1.2, measured on `BlitzyPermissionPolicy.handle()` and the augmented `signInResolver`. The dedicated unit test for the access-audit module is NOT YET CREATED at this checkpoint; functional coverage is provided by the new `auditing.test.ts` E2E suite (see §3.1 and [`docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 8). Four of the six Grafana dashboard panels (Catalog Query Latency, HTTP Error Rate, Node.js Heap, Service Health) populate against the auto-instrumented metrics today; the remaining two (Audit Events Per Minute, Permission Decisions) display "No data" pending the planned custom counters (see §0 row "Custom Prometheus counters").
...
- Prometheus metrics at `http://localhost:9464/metrics` — auto-instrumented HTTP / process / Node.js metrics today. The custom counters originally documented (`blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`) are planned but NOT YET EMITTED at this checkpoint; see §0 row "Custom Prometheus counters". The audit-event channel via `coreServices.auditor` is the operator's authoritative source of truth for sign-in / entity-access / permission-decision signals in the meantime.
...
- **Verify the audit trail captures sign-ins**: sign in via the GitHub provider (or Guest), then tail the backend's stdout to see the structured JSON `user-login` event with `meta.provider`, `meta.username`, `meta.emailDomain`, and the synthetic `meta.correlationId` fields populated. The `user_login_total` Prometheus counter is documented as the contract for future work but NOT YET EMITTED at this checkpoint (see §0 row "Custom Prometheus counters"); the audit-event channel is the operator's authoritative source of truth for sign-in volume in the meantime. When the counter lands, the additional verification command will be `curl -s http://localhost:9464/metrics | grep '^user_login_total'`.
...
| `http://localhost:9464/metrics`                        | Prometheus metrics endpoint (auto-instrumented metrics today; planned custom counters pending — see §0)                                                                                        |
...
| Auto-instrumented counters not visible                                     | Permission framework not enabled, or new plugin modules not registered       | Confirm `permission.enabled: true` in `app-config.yaml`; confirm the new plugin modules are registered in `packages/backend/src/index.ts`. NOTE: the planned custom counters (`blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`) are NOT YET EMITTED at this checkpoint (see §0 row "Custom Prometheus counters"); inspect the structured audit-event log instead until they land.           |
