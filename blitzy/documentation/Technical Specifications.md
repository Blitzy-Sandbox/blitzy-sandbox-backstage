...
### IR-5. Custom Prometheus counters are planned but not yet implemented
...
- **As implemented:** None of the three custom counters are emitted by the source code at this checkpoint. The OpenTelemetry auto-instrumentations (`http_server_request_duration_seconds`, `http_server_requests_total`, `process_resident_memory_bytes`, etc.) ARE emitted; the audit-event channel via `coreServices.auditor` IS emitted; but the three documented custom counters have no registration or increment call in `policy.ts`, `authModuleGithubProvider.ts`, or the access-audit `module.ts`. The `@opentelemetry/api-metrics` package referenced in earlier text is not installed.
- **Status:** The three custom counters are tracked as a future deliverable in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1. Until they are wired in, the audit-event channel is the operator's authoritative source for sign-in / entity-access / permission-decision signals, and the Grafana dashboard panels backed by these counters render "No data" by design.
...
Per-checkpoint verification status of GitHub checks, custom-counter emission, and documentation linting is reported in the companion [`./Project Guide.md`](./Project%20Guide.md) under "Verification Status". Claims in this Technical Specifications document about "all checks passing", "all rules satisfied", or "complete" should be cross-referenced with the Project Guide's measured status before relying on them. At the time of the current review remediation, several rules are PARTIAL (R1 due to IR-5; R2 due to LocalGCP onboarding details; R3 due to nonexistent file references being remediated; R4 due to the catalog count diagram now corrected; R6 due to IR-8); the only fully PASS rule is R7.
...
| Metrics endpoint                        | `PrometheusExporter` exposes `http://localhost:9464/metrics`; `packages/backend/prometheus.yml` defines scrape config; `getNodeAutoInstrumentations()` emits HTTP/runtime metrics automatically             | REUSE the exporter and auto-instrumented metrics. The AAP "ADD — named counters for `user_login_total`, `entity_access_total`, `blitzy_permission_decisions_total` via OTel meters in the new modules" is **planned but not yet implemented** (no `@opentelemetry/api-metrics` import or `Counter.add(...)` call exists in `plugins/permission-backend-module-blitzy-policy/src/policy.ts`, `packages/backend/src/authModuleGithubProvider.ts`, or `plugins/catalog-backend-module-access-audit/src/module.ts`). See IR-5 above. The counter implementation is tracked as a deferred follow-up in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1. |
...
> - `plugins/catalog-backend-module-access-audit/src/module.test.ts` — NOT CREATED at this checkpoint; functional coverage is achieved via `packages/app/e2e-tests/auditing.test.ts`; the dedicated unit test is tracked in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 8
...
> ⚠ **Implementation Reality** — The "Application to this refactor" narrative below was authored against the AAP plan and contains two material divergences from the implementation at this checkpoint. See **IR-5** (custom Prometheus counters are planned, not yet implemented; `@opentelemetry/api-metrics` is not installed) and **IR-6** (health endpoints are `/.backstage/health/v1/{liveness,readiness}` via `coreServices.rootHealth`, not `/health` / `/readiness` via `coreServices.rootHealthService`) at the top of this document. The corrected narrative is summarized at the end of this section.
...
- **Added:**
  ...
  - **PLANNED — not yet implemented at this checkpoint** — Custom Prometheus counters `blitzy_permission_decisions_total{result, email_domain, action}`, `user_login_total{provider, email_domain}`, and `entity_access_total{kind, action}`. The AAP plan was to register these via `@opentelemetry/api-metrics`, but that package is not installed and no registration / increment code exists in `policy.ts`, `authModuleGithubProvider.ts`, or the access-audit `module.ts`. Wiring them into their planned emission sites is tracked as [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1 (see IR-5). Until they land, the audit-event channel is the operator's authoritative source of truth for sign-in / entity-access / permission-decision volume.
  - `docs/observability/dashboards.md` documenting structured logging, correlation ID propagation, OpenTelemetry trace export, the Prometheus metrics catalog (auto-instrumented metrics that ARE emitted today vs planned custom counters), the canonical `/.backstage/health/v1/*` health endpoints, and dashboard import instructions.
  - `docs/observability/dashboard-template.json` — Grafana dashboard template importable via Grafana UI. Four panels (Catalog Query Latency, HTTP Error Rate, Node.js Heap, Service Health) are functional today against the auto-instrumented metrics. Two panels (Audit Events Per Minute, Permission Decisions) are pending the planned custom counters and will return "No data" until the counters land.
- **Local verification:** the LocalGCP emulators allow running the full stack locally; `yarn start` exposes `/metrics` on port 9464, `/.backstage/health/v1/liveness` and `/.backstage/health/v1/readiness` on the backend port, and the audit log emits to stdout in structured JSON. Mermaid traces are visible in the Jaeger UI if exported (optional). The auto-instrumented Prometheus metrics (`http_server_request_duration_seconds_bucket`, `http_server_requests_total`, `process_resident_memory_bytes`, `nodejs_heap_size_used_bytes`) are visible via `curl http://localhost:9464/metrics`. The planned custom counters are NOT yet emitted at this checkpoint and cannot be locally verified until IR-5 work item 1 lands.
...
OpenTelemetry SDK (`@opentelemetry/sdk-node`) is wired in `packages/backend/src/instrumentation.js` with auto-instrumentations (`@opentelemetry/auto-instrumentations-node`) and a Prometheus exporter on `localhost:9464/metrics`. The auto-instrumentations emit HTTP / Express / process / Node-runtime metrics out-of-the-box (`http_server_request_duration_seconds_bucket`, `http_server_requests_total`, `process_resident_memory_bytes`, `nodejs_heap_size_used_bytes`, etc.); these are the metrics that Grafana panels 3-6 of the dashboard template consume.
...
Audit events (`user-login`, `entity-access`) are emitted via `coreServices.auditor` (Workstream C). The audit-event channel is the operator's authoritative source of truth for sign-in / entity-access signals at this checkpoint. The planned custom Prometheus counters (`blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`) are documented as a contract for future work but are **NOT YET REGISTERED OR EMITTED** at this checkpoint — see IR-5 in the Implementation Reality Addendum and `docs/refactor/next-tasks.md` entry 1.
...
Events land in the backend's structured JSON log on stdout and (if configured) in the AuditorService's auxiliary log channel. The audit-event channel is the operator's authoritative source of truth for sign-in / entity-access signals at this checkpoint, because the planned Prometheus counters covering these signals are not yet implemented (see §6.4.4 below and IR-5).
...
#### Planned Custom Prometheus Counters (NOT YET IMPLEMENTED)

> **Implementation status (IR-5):** None of the three counters below are emitted by the source code at this checkpoint. The `@opentelemetry/api-metrics` package is not installed and none of the listed source files contain counter registration or increment calls. The contract is preserved here so that when the counters are wired into their planned emission sites — tracked as [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 1 — the Grafana dashboard (`docs/observability/dashboard-template.json`) and the operator runbooks (`docs/observability/dashboards.md`) become functional automatically without further documentation changes.

| Counter                             | Labels                                                                                                                                                                    | Planned source                                                                                                                      |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `blitzy_permission_decisions_total` | `result` ∈ {ALLOW, DENY}, `email_domain` (bucketed: `blitzy.com`, `external`, `guest`, `unknown`), `action` ∈ {read, create, update, delete, refresh, register, validate} | Inside `BlitzyPermissionPolicy.handle()` at `plugins/permission-backend-module-blitzy-policy/src/policy.ts` (not yet instrumented). |
| `user_login_total`                  | `provider` ∈ {github, guest}, `email_domain` (bucketed)                                                                                                                   | Inside the augmented `signInResolver` at `packages/backend/src/authModuleGithubProvider.ts` (not yet instrumented).                 |
| `entity_access_total`               | `action` ∈ {read}                                                                                                                                                         | Inside the access-audit middleware at `plugins/catalog-backend-module-access-audit/src/module.ts` (not yet instrumented).           |

Auto-instrumented metrics emitted today (used by Grafana panels 3-6 in the template) include `http_server_request_duration_seconds_bucket`, `http_server_requests_total`, `process_resident_memory_bytes`, `nodejs_heap_size_used_bytes`, and Prometheus `up{}` scrape liveness. These cover the catalog query latency panel, HTTP error rate panel, Node.js heap panel, and service health panel of the dashboard template.

#### Cardinality Discipline

To avoid Prometheus cardinality explosion when the planned counters are wired in:

- `email_domain` is bucketed into a small fixed set (`blitzy.com`, `external`, `guest`, `unknown`); the full email address is NEVER a label.
