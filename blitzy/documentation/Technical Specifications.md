...
>
> **Changelog**: Section 0 replaced with canonical AAP for the chrome refactor / `BlitzyPermissionPolicy` / audit trail / catalog count fix PR. Section 1.2, 3.2, 6.4 added to describe current system state.
...
OpenTelemetry SDK (`@opentelemetry/sdk-node`) is wired in `packages/backend/src/instrumentation.js` with auto-instrumentations (`@opentelemetry/auto-instrumentations-node`) and a Prometheus exporter on `localhost:9464/metrics`.
...
Audit events (`user-login`, `entity-access`) are emitted via `coreServices.auditor` (Workstream C). Custom Prometheus counters (`blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`) are registered via the OpenTelemetry meter API.

### 1.2.4 Catalog Plugin Behavior

The catalog backend persistence layer is unchanged. The frontend filter semantics for tags are corrected by Workstream D: `EntityTagFilter.getCatalogFilters()` in `plugins/catalog-react/src/filters.ts` now emits an AND-compatible filter shape so the catalog count under multi-tag selection matches the rendered row count. The catalog data model (`@backstage/catalog-model`) is unchanged; `System`, `Owner`, `Group`, and `Component` relations remain present in the database, but their UI surfaces (catalog Type column, About card Owner / System fields, EntityLayout HeaderLabel Owner) have been removed by Workstream B.
...
| `@backstage/frontend-plugin-api` | workspace pin | `createFrontendModule`, layout blueprints |
...
No MUI v4 → shadcn migration is advanced by this refactor (per AAP §0.3.2 explicit out-of-scope and `docs/refactor/next-tasks.md` entry 1).
...
1. **GitHub OAuth** via `@backstage/plugin-auth-backend-module-github-provider`. The `signInResolver` in `packages/backend/src/authModuleGithubProvider.ts` has been augmented to:
   - Extract email via the priority chain: `result.fullProfile.emails?.find(e => e.primary)?.value` (or `[0].value`), then `result.userinfo?.email`, then fallback to `${username}@unknown.invalid` (the policy treats unknown domain as non-Blitzy).
   - Hydrate `BackstageIdentityResponse.profile.email` so downstream policy reads it server-side without a second catalog lookup.
   - Emit a `user-login` audit event via `coreServices.auditor` with `severityLevel: 'medium'`, `meta: { provider: 'github', username, emailDomain, userEntityRef }`, then `.success({ meta: { entityRef } })` on success or `.fail({ error, meta })` on resolver failure.
   - Wrap the resolver body in try/catch to ensure audit emission on both success and failure paths.

2. **Guest sign-in** via `packages/app/src/GuestSignInPage.tsx` (frontend) and `guestSignInPageModule` (registered in `App.tsx` features array). The Guest principal is identified by `user?.principal?.type === 'guest'` or by the entity ref `user:default/guest`. The signInResolver for Guest sessions also emits a `user-login` audit event with `meta: { provider: 'guest' }`.

The policy MUST NOT trust client-asserted email; it reads only from the validated server-side `BackstageIdentityResponse.profile.email`.
...
2. If the user's email ends in `@blitzy.com` (case-insensitive) → return `{ result: AuthorizeResult.ALLOW }` for any permission. Email is sourced from `BackstageIdentityResponse.profile.email`.
3. If the principal is Guest (detected via `user?.principal?.type === 'guest'` OR `userEntityRef === 'user:default/guest'`) → return `{ result: AuthorizeResult.DENY }` for write/edit actions.
...
1. **`user-login`** — Emitted on every sign-in (GitHub and Guest).
   - Source: `packages/backend/src/authModuleGithubProvider.ts` (augmented `signInResolver`) and Guest provider augmentation.
   - `severityLevel: 'medium'`
   - `meta: { provider: 'github' | 'guest', username, emailDomain, userEntityRef }`
   - Emitted via `.success({ meta: { entityRef } })` on success and `.fail({ error, meta })` on resolver failure.

2. **`entity-access`** — Emitted on every user-credentialed entity read.
   - Source: `plugins/catalog-backend-module-access-audit/src/module.ts` (new backend module that wraps `CatalogService.getEntityByRef` and the catalog query endpoints).
   - `severityLevel: 'low'`
   - `meta: { entityRef, principal, action: 'read' }`
   - Emitted via `.success()` after the catalog returns the entity.

Both event types are correlated with the request's correlation ID via the `AuditorService` implementation. Events land in the backend's structured JSON log on stdout and (if configured) in the AuditorService's auxiliary log channel.
...
| Counter | Labels | Source |
|---|---|---|
| `blitzy_permission_decisions_total` | `result` ∈ {ALLOW, DENY}, `email_domain` (bucketed: `blitzy.com`, `external`, `guest`, `unknown`), `action` ∈ {read, create, update, delete, refresh, register, validate} | `plugins/permission-backend-module-blitzy-policy/src/policy.ts` |
| `user_login_total` | `provider` ∈ {github, guest}, `email_domain` (bucketed) | `packages/backend/src/authModuleGithubProvider.ts`, Guest provider augmentation |
| `entity_access_total` | `action` ∈ {read} | `plugins/catalog-backend-module-access-audit/src/module.ts` |
...
The Grafana dashboard template at `docs/observability/dashboard-template.json` visualizes:
1. Audit events per minute (stacked: `user_login_total` + `entity_access_total`)
2. Permission decisions (ALLOW / DENY split with red/green color overrides)
3. Catalog query latency p50/p95/p99
4. HTTP error rate
5. Node.js heap usage
6. Service health (single-stat `up{job="blitzy-backstage"}`)
...
- The `BackstageIdentityResponse.profile.email` value is server-trusted only; client tampering is not possible because the resolver runs server-side.
