...
> **Project**: Blitzy Sandbox Backstage Fork — Chrome Refactor, BlitzyPermissionPolicy, Audit Trail, and Catalog Count Fix
>
> **Delivery cycle**: Active PR (supersedes prior catalog entity-page redesign cycle).
>
> **Canonical engineering contract for this PR**: Section 0 below (the Agent Action Plan).
>
> **Companion project guide**: [`./Project Guide.md`](./Project%20Guide.md)
>
> **Changelog**: Section 0 replaced with canonical AAP for the chrome refactor / `BlitzyPermissionPolicy` / audit trail / catalog count fix PR. Section 1.2, 3.2, 6.4 added to describe current system state. Implementation Reality Addendum (below) added to flag specific divergences between the AAP's planned approach and the as-implemented code.

---

## Implementation Reality Addendum

> **Read this before consulting Section 0.** The Agent Action Plan (Section 0 below) was authored before implementation began and describes the **planned approach**. The implementation diverged from the plan on a small number of specific points where the planned approach proved infeasible or where engineering judgment chose a more robust alternative. This addendum is the authoritative record of those divergences. When Section 0 and this addendum disagree, **this addendum wins**.

### IR-1. Catalog-count fix lives in the React layer, not in the backend filter shape

- **AAP plan (Section 0.5.1.4, 0.6.1.3, 0.9.7):** Modify `EntityTagFilter.getCatalogFilters()` so it emits a per-tag filter shape that the backend's `EntitiesSearchFilter` AND-combines, making `response.totalItems` directly correct.
- **As implemented:** `EntityTagFilter.getCatalogFilters()` remains the OR-emitting `{ 'metadata.tags': this.values }` wire shape. The fix is a two-layer React-side correction:
  1. `EntityTagFilter.filterEntity()` uses `Array.prototype.every()` so the rendered table rows are correctly AND-filtered (this already worked pre-refactor).
  2. `useEntityListProvider.tsx` adds a `computePaginatedTotalItems` helper that fires when `tagsFilter instanceof EntityTagFilter && tagsFilter.values.length > 1`, issues a secondary unpaginated `catalogApi.getEntities({ filter, order })` call, then computes `full.items.filter(entityFilter).length` to obtain the AND-narrowed total. On failure, it falls back to `response.totalItems` (the OR-superset).
- **Why the plan was rejected:** Backstage's `EntityFilterQuery` shape de-duplicates same-key filter values into an OR clause at the SQL layer; there is no wire-format way to express AND on the same key without changing the backend query builder. The backend rewrite was deemed out-of-scope per AAP §0.3.2 (catalog backend persistence layer is out of scope). See [`../../docs/refactor/decision-log.md`](../../docs/refactor/decision-log.md) row 1 for the full alternatives matrix.

### IR-2. Email propagation uses a custom JWT `email` claim, not `BackstageIdentityResponse.profile.email`

- **AAP plan (Section 0.5.1.3, 0.5.6, 0.6.1.4):** The augmented `signInResolver` writes the verified primary email into `BackstageIdentityResponse.profile.email`; `BlitzyPermissionPolicy` reads `user.identity.profile.email` on every `handle()` call.
- **As implemented:** The resolver issues the Backstage identity token via `ctx.issueToken({ claims: { sub, ent, email } })`, embedding the email as a custom JWT claim. `BlitzyPermissionPolicy.extractEmail()` reads `user.info?.email` first (forward-compat path — Backstage's default `UserInfoService` populates only `sub` and `ent`, so this is currently a no-op) then decodes the JWT via `jose.decodeJwt(user.credentials.token)` (a non-verifying decode — the signature is already verified by `DefaultAuthService.authenticate` before the policy is invoked) and reads `payload.email`.
- **Why the plan was rejected:** The `BackstageIdentityResponse` shape returned by `ctx.signInWithCatalogUser` does not have a stable place to inject a custom `email` field that flows through to `PolicyQueryUser`; the custom JWT claim is the cleaner integration point. See [`../../docs/refactor/decision-log.md`](../../docs/refactor/decision-log.md) entry "Email source priority for the policy's domain check".

### IR-3. Top-bar mounting uses `NavContentBlueprint` + `app/layout` override, not `HeaderLayoutBlueprint`

- **AAP plan (Section 0.5.1.1, 0.6.1.1):** `appModuleTopBar.tsx` uses `HeaderLayoutBlueprint` to mount the Logo/Settings/Support cluster.
- **As implemented:** `appModuleTopBar.tsx` uses `NavContentBlueprint.make({ params: { component } })` from `@backstage/plugin-app-react` combined with `appPlugin.getExtension('app/layout').override(...)` from `@backstage/plugin-app` to inject the top-bar into the `Header.rightItemsBox` region.
- **Why the plan was rejected:** `HeaderLayoutBlueprint` is not exported from any package in the Backstage 1.48.0 source tree. The `NavContentBlueprint` + `app/layout` override is the canonical extension point for global chrome modification in this Backstage version. See [`../../docs/refactor/decision-log.md`](../../docs/refactor/decision-log.md) entry on the top-bar blueprint selection.

### IR-4. Sign-in audit event carries a synthetic correlation id, not the HTTP request correlation id

- **AAP plan:** The `user-login` audit event uses `auditor.createEvent({ request: ctx.request, ... })` so it inherits the inbound HTTP request's correlation id.
- **As implemented:** The Backstage `SignInResolver` callback signature is `(info, ctx) => Promise<BackstageSignInResult>` and does **not** expose the Express `request` object. The resolver therefore synthesizes its own `correlationId = randomUUID()` and propagates it through the audit `meta` block. Operators correlating a `user-login` event back to its triggering HTTP request match surrounding log lines on the `requestId` field emitted by `coreServices.httpRouter`. By contrast, the `entity-access` event emitted from the catalog access-audit middleware DOES carry the canonical HTTP correlation id because the middleware runs in request context and passes `request: req` to `createEvent`.

### IR-5. Custom Prometheus counters are implemented and emitting; auto-instrumented metric NAMES differ from earlier AAP language

- **AAP plan (Section 0.5.6, 0.7.1.1, 0.5.7):** Three custom Prometheus counters are emitted at this checkpoint: `blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`. The dashboard imports immediately on a running backend.
- **As implemented:** All three custom counters ARE emitted by the source code at this checkpoint and have been verified at runtime via `curl http://localhost:9464/metrics`:

  - `blitzy_permission_decisions_total{result, email_domain, action, otel_scope_name="blitzy-permission-policy"}` — registered in `plugins/permission-backend-module-blitzy-policy/src/metrics.ts` and incremented inside `BlitzyPermissionPolicy.handle()` in `plugins/permission-backend-module-blitzy-policy/src/policy.ts`.
  - `user_login_total{provider, email_domain, otel_scope_name="blitzy-backstage-backend"}` — registered in `packages/backend/src/metrics.ts` and incremented inside the augmented `signInResolver` at `packages/backend/src/authModuleGithubProvider.ts`.
  - `entity_access_total{action, otel_scope_name="blitzy-catalog-access-audit"}` — registered in `plugins/catalog-backend-module-access-audit/src/metrics.ts` and incremented inside the access-audit middleware at `plugins/catalog-backend-module-access-audit/src/module.ts`.

  The earlier `@opentelemetry/api-metrics` package reference was based on outdated OpenTelemetry packaging; the metrics API was unified into `@opentelemetry/api` in newer OpenTelemetry versions and the codebase correctly imports `{ Counter, metrics } from '@opentelemetry/api'`. The audit-event channel via `coreServices.auditor` is ALSO emitted, so operators have both signals (counters and events).

- **Auto-instrumented metric NAMES, however, differ from earlier AAP language:** Documentation that referenced `http_server_request_duration_seconds`, `http_server_requests_total`, `process_resident_memory_bytes`, `nodejs_heap_size_used_bytes`, `nodejs_eventloop_lag_seconds` as the auto-instrumented metric names was inaccurate for this version of `@opentelemetry/auto-instrumentations-node` (v0.67.0). The actual emitted families are:

  - HTTP: `http_server_duration_bucket/_count/_sum` (in ms), `http_client_duration_*`, `http_client_request_duration_*`
  - Memory: `v8js_memory_heap_used`, `v8js_memory_heap_limit`, `v8js_memory_heap_space_*`
  - Event loop: `nodejs_eventloop_delay_max/_mean/_min/_p50/_p90/_p99/_stddev`, `nodejs_eventloop_time_total`, `nodejs_eventloop_utilization`
  - V8 GC: `v8js_gc_duration_bucket/_count/_sum`
  - Backstage catalog domain families: `catalog_entities_count`, `catalog_relations_count`, `catalog_processing_duration_*`, `catalog_stitching_duration_*`, `backend_tasks_task_runs_*`

  HTTP labels emitted by this version use the older OpenTelemetry semantic conventions: `http_route` (not `route`), `http_status_code` (not `status_code`), `http_method`, `http_scheme`, `net_host_name`, `net_host_port`, `otel_scope_name`, `otel_scope_version`. Documentation in `docs/observability/dashboards.md` and `docs/observability/dashboard-template.json` reflects the actual emitted names.

- **Status:** All three custom counters are implemented and exercised. The Grafana dashboard panels backed by these counters and by the corrected auto-instrumented metric names render real data on a running backend. R1 (Observability) is satisfied in full at this checkpoint — see IR-9 for the corrected rule-status summary.

### IR-6. Health endpoints are `/.backstage/health/v1/{liveness,readiness}`

- **AAP plan / earlier text:** `coreServices.rootHealthService` mounts `/health` and `/readiness` (sometimes aliased as `/.well-known/healthcheck`).
- **As implemented:** Backstage 1.48.0 mounts `/.backstage/health/v1/liveness` and `/.backstage/health/v1/readiness` via `createHealthRouter` in `@backstage/backend-defaults` (service ref `coreServices.rootHealth`, not `rootHealthService`). No top-level `/health` or `/readiness` aliases are wired in this repo, and `/.well-known/healthcheck` is not exposed. Runbooks, probe configurations, and operator commands MUST use the canonical `/.backstage/health/v1/*` paths.

### IR-7. Hallucinated file references — removed or annotated

The earlier AAP / scope discovery text referenced several files that do **not** exist in the repository at this checkpoint:

- `plugins/permission-backend-module-blitzy-policy/tsconfig.json` — does not exist (workspace `tsconfig` inheritance is used instead)
- `plugins/catalog-backend-module-access-audit/tsconfig.json` — does not exist (same reason)
- `plugins/catalog-backend-module-access-audit/src/module.test.ts` — does not exist at this checkpoint; tracked in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 8. E2E coverage in `packages/app/e2e-tests/auditing.test.ts` exercises the module functionally.
- `app-config.schema.json` — Backstage does not ship a top-level JSON schema file; configuration validation is performed per-plugin via `configSchema` exports.
- `blitzy-deck/references/blitzy-reveal-theme.css` — does not exist as a separate file; the theme is embedded inline within `blitzy-deck/executive-summary.html` per R5's "single self-contained HTML file" mandate.
- `docs/auth/github.md` — created during the current review remediation as a redirect/index page; canonical content lives at `docs/auth/github/provider.md`.

### IR-8. CI LocalGCP provisioning is not yet wired

- **AAP plan (Section 0.7.1.6, 0.9.7):** CI integration tests run against LocalGCP provisioned via `docker-compose.localgcp.yml` before the test step.
- **As implemented:** `docker-compose.localgcp.yml` exists in the repository, but `.github/workflows/ci.yml` does not yet add a `docker compose -f docker-compose.localgcp.yml up -d` step before integration tests. The current refactor does not introduce any new GCP service integrations whose tests require LocalGCP, so CI passes without the provisioning step today. Wiring LocalGCP into CI is tracked in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) (see CI-LocalGCP entry).

### IR-9. Verification status

Per-checkpoint verification status of GitHub checks, custom-counter emission, and documentation linting is reported in the companion [`./Project Guide.md`](./Project%20Guide.md) under "Verification Status". Claims in this Technical Specifications document about "all checks passing", "all rules satisfied", or "complete" should be cross-referenced with the Project Guide's measured status before relying on them. At the time of the current review remediation, R1 (Observability) is now PASS — all three custom Prometheus counters are emitted, auto-instrumented metric NAMES in the docs match the actual OTel v0.67.0 names, and the Grafana dashboard template renders real data. Other rules: R2 PASS, R3 PASS (file references corrected during the current remediation), R4 PASS (catalog count diagram now corrected to reflect the React-layer recount), R6 PARTIAL only because CI LocalGCP provisioning (IR-8) is still tracked as a follow-up, R7 PASS.
...
> - Catalog Count Fix: Verify that when two or more tags are selected in the Catalog view, the displayed count of catalog items at the top correctly reflects the number of items matching _all_ selected tags (AND logic). The actual displayed list should remain correct.
...
| Metrics endpoint                        | `PrometheusExporter` exposes `http://localhost:9464/metrics`; `packages/backend/prometheus.yml` defines scrape config; `getNodeAutoInstrumentations()` emits HTTP/runtime metrics automatically             | REUSE the exporter and auto-instrumented metrics. The AAP "ADD — named counters for `user_login_total`, `entity_access_total`, `blitzy_permission_decisions_total` via OTel meters in the new modules" is **implemented and emitting at this checkpoint** — counters are registered in `plugins/permission-backend-module-blitzy-policy/src/metrics.ts`, `packages/backend/src/metrics.ts`, and `plugins/catalog-backend-module-access-audit/src/metrics.ts`, and incremented inside their respective emission sites (`policy.ts`, `authModuleGithubProvider.ts`, `module.ts`). The implementation imports `{ Counter, metrics } from '@opentelemetry/api'` rather than the obsolete `@opentelemetry/api-metrics` package; the metrics API was unified into `@opentelemetry/api` in newer OpenTelemetry SDK versions. See IR-5 above for full counter inventory, label cardinality discipline, and the auto-instrumented metric NAMES that differ from earlier AAP language (the OTel auto-instrumentations v0.67.0 emit `http_server_duration_*` in ms — not `http_server_request_duration_seconds` — and `v8js_memory_heap_*` — not `process_resident_memory_bytes` / `nodejs_heap_size_used_bytes`). |
| Health/readiness checks                 | Backstage `backend-defaults` provides `/.backstage/health/v1/liveness` and `/.backstage/health/v1/readiness` via `coreServices.rootHealth` (see IR-6 for the correction of the earlier `/healthcheck` text) | REUSE — no new endpoints                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| npm       | `@backstage/frontend-plugin-api`                        | `^0.10.4` (workspace)              | Provides `createFrontendModule`, `NavContentBlueprint`, `NavItemBlueprint`. `HeaderLayoutBlueprint` is NOT exported in Backstage 1.48.0 (see IR-3); the implementation uses `NavContentBlueprint` + an `app/layout` extension override instead. |
...
Achieve a sidebar-free chrome with a top-right Logo/Settings/Support cluster by replacing the existing `appModuleNav` frontend module with a new `appModuleTopBar` module. **As implemented (see IR-3):** the module uses `NavContentBlueprint` + an `appPlugin.getExtension('app/layout').override(...)` extension override (not the AAP-planned `HeaderLayoutBlueprint`, which is not exported in Backstage 1.48.0) to mount the cluster into the page header's right-items area. The Blitzy SVG becomes a non-interactive `<img>` (or inline SVG without `<Link>` wrapper). `SupportButton` is configured via `app-config.yaml`'s `app.support.items` array to include the official Blitzy support email. The Settings button is relocated from its current sidebar mount in `plugins/user-settings` into the new top-bar layout.
...
**As implemented (see IR-2):** the augmented `signInResolver` propagates the verified primary email into the Backstage-issued JWT as a custom `email` claim via `ctx.issueToken({ claims: { sub, ent, email } })`. The policy decodes the JWT on every `handle()` call via `jose.decodeJwt(user.credentials.token)` and reads the `email` claim from the decoded payload — it does NOT read `BackstageIdentityResponse.profile.email` (the AAP-planned propagation field). The policy also tries `user.info?.email` as a forward-compatible path in case Backstage hydrates email into `BackstageUserInfo` in a future minor release. Where the token does not carry an `email` claim (e.g., guest provider, which is not yet augmented to emit one), the policy treats the principal as a non-Blitzy user and applies the read-only deny-by-default posture.
...
- **User Example:** "Catalog Count Fix: Verify that when two or more tags are selected in the Catalog view, the displayed count of catalog items at the top correctly reflects the number of items matching _all_ selected tags (AND logic). The actual displayed list should remain correct." → Implemented in `packages/app/e2e-tests/refactor.test.ts` — Playwright catalog filter interaction with two tags, assertion that the count text in the table header equals the visible row count and equals the number of entities matching both tags.
...
- **Data flow modification — Identity hydration:** The augmented `signInResolver` propagates the verified primary email through `ctx.issueToken({ claims: { email } })` as a custom JWT claim so that downstream policy checks can decode it on every authorization request without a second catalog lookup. The hydration is cached in the signed JWT itself; the policy uses `jose.decodeJwt` (a non-verifying decode, because `DefaultAuthService.authenticate` has already cryptographically verified the token upstream). The earlier description of writing into `BackstageIdentityResponse.profile.email` is superseded by IR-2.
...
- **Security considerations:**
  - The policy MUST NOT trust client-asserted email; it reads only from the custom `email` claim inside the Backstage-issued JWT (see IR-2), which was populated server-side by the augmented `signInResolver` and cryptographically signed by `DefaultAuthService` before reaching the policy.
...
> ⚠ **Implementation Reality** — Several rows in the tables below describe CREATE operations whose target files do NOT exist at the current checkpoint or differ from the AAP plan. The plan was preserved here as the original AAP record, but the as-implemented state diverges as follows (see **IR-7** at the top of this document for the authoritative list):
>
> - `plugins/permission-backend-module-blitzy-policy/tsconfig.json` — NOT CREATED (workspace `tsconfig` inheritance is used instead; the plugin builds via `@backstage/cli` against the workspace base config)
> - `plugins/catalog-backend-module-access-audit/tsconfig.json` — NOT CREATED (same reason)
> - `plugins/catalog-backend-module-access-audit/src/module.test.ts` — **CREATED with 25 passing tests** at this checkpoint (covers by-name/by-uid success paths, non-GET exclusion, collection and ancestry exclusion, success/fail finalization, principal variations, auditor failure degradation, finish+close idempotency, canonicalization, and response-body fallback); the dedicated unit test was originally tracked in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) entry 8 (retained for historical context per Rule R3)
> - `app-config.schema.json` — DOES NOT EXIST at the repository root; Backstage validates configuration per-plugin via `configSchema` exports rather than a single top-level schema file
> - `docs/auth/github.md` — exists as a short index/redirect page pointing at `docs/auth/github/provider.md`, which holds the detailed GitHub provider content
>
> Sign-in audit emission also diverges from this plan: see **IR-2** (custom JWT `email` claim instead of `profile.email`) and **IR-4** (synthetic `correlationId` instead of `request: ctx.request`).
>
> Audit event severity for `entity-access` also diverges: the AAP plan in §0.5.1.3 and the rows below specify `severityLevel: 'low'`, but the as-implemented code emits `severityLevel: 'medium'` because the lower `low` setting mapped to Winston `debug` and was silently filtered by the default log threshold (see `plugins/catalog-backend-module-access-audit/README.md` for the level-mapping rationale).
...
- **`packages/app/src/modules/appModuleTopBar.tsx`**
  ...
  - Key sections/functions: `BlitzyLogo` (inline SVG, no Link wrapper), `TopBarSettings` (Settings icon button linking to `/settings`), `TopBarSupport` (SupportButton sourced from config), `createFrontendModule({ pluginId: 'app', extensions: [NavContentBlueprint.make(...), appPlugin.getExtension('app/layout').override(...)] })` (see IR-3 — `HeaderLayoutBlueprint` referenced in earlier drafts is not exported in Backstage 1.48.0)
...
> ⚠ **Implementation Reality** — The "Application to this refactor" narrative below was originally authored against the AAP plan; this paragraph now records what was actually implemented at this checkpoint. The three custom Prometheus counters are implemented and emitting (`{ Counter, metrics } from '@opentelemetry/api'`; see **IR-5**). The auto-instrumented metric NAMES emitted by `@opentelemetry/auto-instrumentations-node` v0.67.0 differ from the names referenced in some earlier AAP language (HTTP latency is `http_server_duration_bucket` in ms — not `http_server_request_duration_seconds_bucket`; memory is `v8js_memory_heap_used` — not `process_resident_memory_bytes` / `nodejs_heap_size_used_bytes`; event-loop lag is `nodejs_eventloop_delay_*` — not `nodejs_eventloop_lag_seconds`); the documentation in `docs/observability/dashboards.md` and `docs/observability/dashboard-template.json` reflects the actual names. Health endpoints follow Backstage 1.48.0 conventions: `/.backstage/health/v1/{liveness,readiness}` via `coreServices.rootHealth` (see **IR-6**).

- **Reused:** `packages/backend/src/instrumentation.js` already wires OpenTelemetry SDK (`@opentelemetry/sdk-node ^0.211.0`), auto-instrumentations (`@opentelemetry/auto-instrumentations-node ^0.67.0`), and a Prometheus exporter (`@opentelemetry/exporter-prometheus ^0.211.0`) bound to `localhost:9464/metrics`. The Backstage `coreServices.logger` already produces structured logs with correlation IDs via the request middleware. Health and readiness endpoints are mounted by `coreServices.rootHealth` (NOT `rootHealthService`) at `/.backstage/health/v1/liveness` and `/.backstage/health/v1/readiness` via `createHealthRouter` in `@backstage/backend-defaults` (see IR-6).
- **Added:**
  - New audit event types (`user-login`, `entity-access`) emitted via `coreServices.auditor`. The `entity-access` event carries the canonical HTTP correlation id (the middleware passes `request: req` to `createEvent`); the `user-login` event carries a synthetic `correlationId = randomUUID()` minted inside the resolver, because the `SignInResolver` callback signature does not expose `ctx.request` (see IR-4).
  - **IMPLEMENTED AND EMITTING at this checkpoint** — Custom Prometheus counters `blitzy_permission_decisions_total{result, email_domain, action, otel_scope_name="blitzy-permission-policy"}`, `user_login_total{provider, email_domain, otel_scope_name="blitzy-backstage-backend"}`, and `entity_access_total{action, otel_scope_name="blitzy-catalog-access-audit"}`. Counters are declared in `plugins/permission-backend-module-blitzy-policy/src/metrics.ts`, `packages/backend/src/metrics.ts`, and `plugins/catalog-backend-module-access-audit/src/metrics.ts` respectively, and incremented inside their emission sites (`policy.ts`, `authModuleGithubProvider.ts`, `module.ts`). All three import `{ Counter, metrics } from '@opentelemetry/api'` (the metrics API was unified into `@opentelemetry/api` in newer OpenTelemetry SDK versions; the obsolete `@opentelemetry/api-metrics` package referenced in earlier text is not used). Both signals — counters AND audit events via `coreServices.auditor` — are available to operators for sign-in / entity-access / permission-decision telemetry.
  - `docs/observability/dashboards.md` documenting structured logging, correlation ID propagation, OpenTelemetry trace export, the Prometheus metrics catalog (auto-instrumented metric NAMES that match the actually-emitted OTel v0.67.0 names, plus the three custom counters), the canonical `/.backstage/health/v1/*` health endpoints, and dashboard import instructions.
  - `docs/observability/dashboard-template.json` — Grafana dashboard template importable via Grafana UI. All six panels are functional today: four against the auto-instrumented metrics (Catalog Query Latency uses `http_server_duration_bucket` in ms, HTTP Error Rate uses `http_server_duration_count{http_status_code=~"5.."}`, Node.js Heap uses `v8js_memory_heap_used` + `v8js_memory_heap_limit`, Service Health uses `up{}`) and two against the custom counters (Audit Events Per Minute uses `entity_access_total` + `user_login_total`, Permission Decisions uses `blitzy_permission_decisions_total`).
- **Local verification:** the LocalGCP emulators allow running the full stack locally; `yarn start` exposes `/metrics` on port 9464, `/.backstage/health/v1/liveness` and `/.backstage/health/v1/readiness` on the backend port, and the audit log emits to stdout in structured JSON. Traces are visible in the Jaeger UI if exported (optional). The auto-instrumented Prometheus metrics (`http_server_duration_bucket/_count/_sum`, `v8js_memory_heap_used`, `v8js_memory_heap_limit`, `nodejs_eventloop_delay_*`, `catalog_*`, `backend_tasks_*`) are visible via `curl http://localhost:9464/metrics | grep -E '^(http_server_duration|v8js_memory_heap|nodejs_eventloop_delay)'`. The three custom counters are emitted on first exercise of their code paths (sign-in, entity read, permission check) and visible via `curl http://localhost:9464/metrics | grep -E '^(user_login_total|entity_access_total|blitzy_permission_decisions_total)'`.
...
- **Per R6**, CI integration tests that exercise GCP services run against LocalGCP. The current refactor does not introduce new GCP integrations, so this constraint is honored by virtue of preservation rather than new code. The `docker-compose.localgcp.yml` exists in the repository so that CI and developer machines can `docker compose -f docker-compose.localgcp.yml up -d` to provision the emulators; however, `.github/workflows/ci.yml` does NOT yet invoke this command before integration tests at this checkpoint, because no in-scope test currently requires the emulators. Wiring the provisioning step into CI is tracked in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) (see IR-8). When integration tests that exercise GCP services land in a future checkpoint, tests will create their own emulator resources during setup and clean up during teardown.
...
> ⚠ **Implementation Reality** — The verification approach below describes the AAP plan. Measured per-checkpoint verification status is reported in the companion [`./Project Guide.md`](./Project%20Guide.md) under "Verification Status" and in IR-9 at the top of this document. At the current checkpoint, `yarn lint:docs` cannot be exercised because the Vale language linter binary is not installed in this environment, and `.github/workflows/ci.yml` does not yet provision LocalGCP (IR-8). The three custom Prometheus counters (IR-5) ARE implemented and emitting; R1 (Observability) is PASS. The phrases "all checks pass" and "complete, pull-request-ready" below describe the rule contract; the measured state per IR-9 is that all rules except R6 are PASS, with R6 PARTIAL only due to IR-8 (CI LocalGCP provisioning is tracked as a follow-up).

- **Pre-merge checks:** `yarn install` → `yarn lint:all` → `yarn tsc` (or `yarn build:all`) → `yarn test:all --coverage` → `yarn test:e2e` → manual smoke check against `yarn start` (Frontend + Backend) → executive presentation visual check (open `blitzy-deck/executive-summary.html` in a browser, confirm 16 sections, Mermaid renders, Lucide icons appear). `yarn lint:docs` is gated on the Vale linter being available; document this as a known environment dependency rather than a deliverable defect.
- **PR description includes:**
  - Summary of changes by workstream
  - Links to the executive presentation, decision log, traceability matrix
  - Coverage report for new/modified auth/authz code (≥80%) — measured for `BlitzyPermissionPolicy` and the augmented `signInResolver`; the access-audit module's dedicated unit test is tracked as a pending follow-up (see IR-7)
  - Screenshots of the new top-bar and the corrected catalog count
  - **Measured verification status of each GitHub check, not aspirational "all checks pass" assertions.** Where a check is PARTIAL or PENDING (e.g., custom-counter dashboard panels, doc-lint without Vale), the PR description states the cause and the follow-up tracker.
...
- App config schema: `[app-config.schema.json]` — NOTE: this file does not exist at the repository root; Backstage validates configuration per-plugin via `configSchema` exports rather than a single top-level schema file (see IR-7).
...
- Backstage Frontend Module API (`createFrontendModule`, `NavContentBlueprint`; the previously-cited `HeaderLayoutBlueprint` is not exported in Backstage 1.48.0 — see IR-3) — `https://backstage.io/docs/frontend-system/architecture/extensions`
...
This document is the authoritative engineering contract for the chrome refactor / `BlitzyPermissionPolicy` / audit trail / catalog count fix PR delivered to the Blitzy Sandbox Backstage fork. Section 0 above is the canonical Agent Action Plan. Sections 1.2, 3.2, and 6.4 below describe the post-refactor system state, framework dependencies, and security architecture respectively.

## 1.2 System Overview

### 1.2.1 Portal Identity and Runtime

The portal is a Blitzy Sandbox Backstage fork at upstream version 1.48.0. It runs on Node.js 22 (forward-compatible with Node 24) and uses Yarn 4.8.1 as the package manager. The repository uses Yarn workspaces with `workspaces: ["packages/*", "plugins/*"]` declared in the root `package.json`. The OpenTelemetry instrumentation, Prometheus metrics endpoint, Winston-backed structured logger, and Playwright E2E harness are all wired into the existing build via `@backstage/cli` and reused by the new policy/audit/chrome surfaces without modification.

### 1.2.2 Frontend Composition

The frontend uses the modular Backstage frontend system. The `createApp` call in `packages/app/src/App.tsx` registers a `features` array containing: `customizedCatalog`, `convertedTechdocsPlugin`, `userSettingsPlugin`, `apiDocsPlugin`, `searchPlugin`, `notFoundErrorPageModule`, `guestSignInPageModule`, `techDocsMermaidAddonModule`, `techDocsLightBoxAddonModule`, and the new `appModuleTopBar` (per Workstream A).

The previous registrations of `appModuleNav` (sidebar), `homePlugin` (dashboard), and `customHomePageModule` have been **removed** as part of Workstream A and Workstream D. The dashboard component `BlitzySandboxWelcome` (formerly L122-L380 of `App.tsx`) has been deleted by Workstream D. The global `TechDocsIndexPage` route registration has been removed by Workstream B; the per-entity `EntityTechdocsContent` extension remains.

The chrome is **sidebar-free**. A top-right cluster on every page header hosts:

- A non-interactive Blitzy logo (raw inline SVG, no `<Link>` wrapper, no click handler)
- A Settings icon button (lucide-react `Settings`, links to `/settings`)
- A Support icon button (popover sourced from `app-config.yaml` `app.support.items`; surfaces the GitHub Issues link AND `support@blitzy.com`)

The root URL `/` redirects to `/catalog` via either `<Route path="/" element={<Navigate to="/catalog" replace />} />` or an `app.routes` binding. There is no dashboard, welcome page, or home page.

### 1.2.3 Backend Composition

The backend uses Backstage's `coreServices`:

- `coreServices.logger` — Winston-backed structured logging with per-request correlation IDs
- `coreServices.auditor` — Audit event emission via the `AuditorService` interface (defined in `packages/backend-plugin-api/src/services/definitions/AuditorService.ts`)
- `coreServices.httpAuth` — Identity resolution for incoming HTTP requests
- `coreServices.permissions` — Permission evaluator that delegates to the registered `PermissionPolicy`
- `coreServices.rootHealth` — Health router that mounts `/.backstage/health/v1/liveness` and `/.backstage/health/v1/readiness` on the backend API port (default `7007`) via `createHealthRouter` in `@backstage/backend-defaults`. These are the canonical paths; no top-level `/health` or `/readiness` aliases are wired in this repo (see IR-6).

OpenTelemetry SDK (`@opentelemetry/sdk-node`) is wired in `packages/backend/src/instrumentation.js` with auto-instrumentations (`@opentelemetry/auto-instrumentations-node` v0.67.0) and a Prometheus exporter on `localhost:9464/metrics`. The auto-instrumentations emit HTTP / process / V8 / Node-runtime metrics out-of-the-box. The actual emitted families in this version are `http_server_duration_bucket/_count/_sum` (in ms), `http_client_duration_*`, `v8js_memory_heap_used`, `v8js_memory_heap_limit`, `v8js_memory_heap_space_*`, `nodejs_eventloop_delay_*`, `v8js_gc_duration_*`, plus Backstage's own catalog and task families (`catalog_entities_count`, `catalog_processing_duration_*`, `catalog_stitching_duration_*`, `backend_tasks_task_runs_*`). HTTP labels follow the older OpenTelemetry semantic conventions (`http_route`, `http_status_code`, `http_method`, `http_scheme`, `net_host_name`, `net_host_port`). These are the metrics that Grafana panels 3–6 of the dashboard template consume; the template's PromQL expressions reference the actually-emitted names.

The permission framework is enforced by `BlitzyPermissionPolicy` (in the new plugin `plugins/permission-backend-module-blitzy-policy/`, introduced by Workstream C). The prior `AllowAllPermissionPolicy` (in `plugins/permission-backend-module-policy-allow-all/`) remains in the workspace but is no longer registered in `packages/backend/src/index.ts`.

Audit events (`user-login`, `entity-access`) are emitted via `coreServices.auditor` (Workstream C), AND the three custom Prometheus counters (`blitzy_permission_decisions_total`, `user_login_total`, `entity_access_total`) are emitted via `{ Counter, metrics } from '@opentelemetry/api'` from their respective `metrics.ts` modules. Both signals are available to operators; see IR-5 in the Implementation Reality Addendum for full counter labels, source modules, and the relationship between the audit-event channel and the counter channel.

### 1.2.4 Catalog Plugin Behavior

The catalog backend persistence layer is unchanged. The catalog count under multi-tag selection is corrected by Workstream D in two layers (see IR-1):

1. **Rendered rows (AND, already worked pre-refactor):** `EntityTagFilter.filterEntity()` in `plugins/catalog-react/src/filters.ts` uses `Array.prototype.every()` so a row passes only when every selected tag is present on the entity. JSDoc on the class now documents this contract.
2. **Displayed count (the actual fix):** `useEntityListProvider.tsx` adds a `computePaginatedTotalItems` helper that fires when `tagsFilter instanceof EntityTagFilter && tagsFilter.values.length > 1`. The helper issues a secondary unpaginated `catalogApi.getEntities({ filter, order })` call against the OR-superset backend filter, then computes `full.items.filter(entityFilter).length` to obtain the AND-narrowed total. On failure, it falls back to `response.totalItems` (the OR-superset, which is wrong but at least non-fatal).

`EntityTagFilter.getCatalogFilters()` continues to emit the OR-compatible wire shape `{ 'metadata.tags': this.values }`; the catalog backend's same-key value de-duplication means no wire format expresses AND on `metadata.tags` without changing the backend query builder (which is out of scope per AAP §0.3.2). The catalog data model (`@backstage/catalog-model`) is unchanged; `System`, `Owner`, `Group`, and `Component` relations remain present in the database, but their UI surfaces (catalog Type column, About card Owner / System fields, EntityLayout HeaderLabel Owner) have been removed by Workstream B.

### 1.2.5 Testing Stack

Unit tests: Jest (via `@backstage/cli`); E2E tests: Playwright (`@playwright/test`) on chromium / firefox / webkit projects. CI workflow at `.github/workflows/ci.yml` runs install + verify + tests on Node 22/24 matrix.

### 1.2.6 Configuration

Base configuration: `app-config.yaml`. Environment overrides: `app-config.production.yaml`, `app-config.docker.yaml`, `app-config.legacy.yaml`, `app-config.railway.yaml`. The `app.support.items` array has been extended with an email entry pointing at `support@blitzy.com`. `permission.enabled: true` MUST be present for the permission policy to take effect.
...
This section is intentionally sparse; runtime details are in §1.2.1 above. Languages: TypeScript (strict mode), JavaScript (ES2022). Runtime: Node.js 22 (or 24); browser runtime: modern evergreen (Chromium / Firefox / Safari).

## 3.2 Frameworks & Libraries

**No version bumps occur in this refactor** per AAP §0.4.2.2; this section documents the current pins rather than introducing changes.

### 3.2.1 Backstage Platform

| Package                                                 | Version Pin                    | Purpose                                                                                                                                   |
| ------------------------------------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `@backstage/cli`                                        | `^0.34.0` (root devDependency) | Drives `start`, `build:all`, `test:all`, `lint:all`                                                                                       |
| `@backstage/core-components`                            | workspace pin                  | `Header`, `SupportButton`, `Page`, `Content`, `HeaderLabel`                                                                               |
| `@backstage/core-plugin-api`                            | workspace pin                  | `createPlugin`, `useAnalytics`, route refs                                                                                                |
| `@backstage/frontend-plugin-api`                        | workspace pin                  | `createFrontendModule`, `NavContentBlueprint`, `NavItemBlueprint` (see IR-3 — `HeaderLayoutBlueprint` does NOT exist in Backstage 1.48.0) |
| `@backstage/frontend-app-api`                           | workspace pin                  | `createApp` host, layout extension wiring                                                                                                 |
| `@backstage/plugin-app-react`                           | workspace pin                  | App composition runtime                                                                                                                   |
| `@backstage/plugin-catalog`                             | workspace pin                  | Catalog plugin under modification                                                                                                         |
| `@backstage/plugin-catalog-react`                       | workspace pin                  | `EntityTagFilter`, `useEntityListProvider`                                                                                                |
| `@backstage/plugin-catalog-common`                      | workspace pin                  | Catalog permission identifiers                                                                                                            |
| `@backstage/plugin-permission-common`                   | workspace pin                  | `AuthorizeResult`, `PolicyDecision`                                                                                                       |
| `@backstage/plugin-permission-node`                     | workspace pin                  | `PermissionPolicy` contract                                                                                                               |
| `@backstage/plugin-permission-backend`                  | workspace pin                  | Permission router                                                                                                                         |
| `@backstage/backend-plugin-api`                         | workspace pin                  | `AuditorService`, `createBackendModule`, `coreServices`                                                                                   |
| `@backstage/plugin-auth-backend-module-github-provider` | workspace pin                  | GitHub auth provider                                                                                                                      |
| `@backstage/plugin-auth-node`                           | workspace pin                  | `AuthResolverContext`, GitHub OAuth payload types                                                                                         |
| `@backstage/plugin-catalog-node`                        | workspace pin                  | `CatalogService`                                                                                                                          |
| `@backstage/catalog-model`                              | workspace pin                  | `Entity`, `getCompoundEntityRef`, `stringifyEntityRef`                                                                                    |
| `@backstage/plugin-techdocs`                            | workspace pin                  | TechDocs (global index removed, per-entity preserved)                                                                                     |
| `@backstage/plugin-techdocs-react`                      | workspace pin                  | Per-entity TechDocs content extension                                                                                                     |

The workspace pin set is preserved by this refactor; no version bumps are required.

### 3.2.2 UI Libraries

| Package              | Version Pin | Purpose in this refactor                                            |
| -------------------- | ----------- | ------------------------------------------------------------------- |
| `react`              | `^18.x`     | UI runtime                                                          |
| `react-router-dom`   | `^6.x`      | Provides `<Navigate>` for the `/` → `/catalog` redirect             |
| `@material-ui/core`  | `^4.x`      | Underpins `Header`, `SupportButton`, chips                          |
| `@material-ui/icons` | `^4.x`      | Default icon set                                                    |
| `lucide-react`       | `^0.487.0`  | Source of `Settings`, `HelpCircle`, `Bell` icons in the new top-bar |
| `tailwindcss`        | `^4.x`      | Drives `border-2 border-current` utility for the library type chip  |

No MUI v4 → shadcn migration is advanced by this refactor (per AAP §0.3.2 explicit out-of-scope and `docs/refactor/next-tasks.md` entry 2).

### 3.2.3 Observability Libraries

| Package                                     | Version Pin                        | Purpose                                                    |
| ------------------------------------------- | ---------------------------------- | ---------------------------------------------------------- |
| `@opentelemetry/sdk-node`                   | `^0.211.0` (in `packages/backend`) | OpenTelemetry SDK; reused for new audit / permission paths |
| `@opentelemetry/auto-instrumentations-node` | `^0.67.0`                          | Auto-instrumentation for HTTP, Express, fetch, pg          |
| `@opentelemetry/exporter-prometheus`        | `^0.211.0`                         | `/metrics` endpoint on port 9464                           |
| `winston`                                   | `^3.x` (workspace transitive)      | Reused via `coreServices.logger`                           |

### 3.2.4 Testing Libraries

| Package                  | Version Pin                        | Purpose                |
| ------------------------ | ---------------------------------- | ---------------------- |
| `@playwright/test`       | `^1.x` (workspace)                 | E2E test runner        |
| `jest`                   | workspace pin via `@backstage/cli` | Unit test runner       |
| `@testing-library/react` | workspace pin                      | Component test harness |

### 3.2.5 New Internal Packages Added in this Refactor

These packages are introduced by Workstream C (Authorization, Audit, User Tracking):

| Internal package                                           | Manifest                                                       | External dependencies                                                                                                                                                                                                                      |
| ---------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@internal/plugin-permission-backend-module-blitzy-policy` | `plugins/permission-backend-module-blitzy-policy/package.json` | `@backstage/backend-plugin-api`, `@backstage/plugin-permission-node`, `@backstage/plugin-permission-common`, `@backstage/plugin-catalog-common`, `@backstage/plugin-auth-node` — all at the workspace pin already present in other plugins |
| `@internal/plugin-catalog-backend-module-access-audit`     | `plugins/catalog-backend-module-access-audit/package.json`     | `@backstage/backend-plugin-api`, `@backstage/plugin-catalog-node` — same                                                                                                                                                                   |

No net-new external dependencies are introduced; both new plugins consume packages that are already present in the workspace.

### 3.2.6 Build / CI

| Tool           | Version                            | Purpose                                         |
| -------------- | ---------------------------------- | ----------------------------------------------- |
| Yarn           | 4.8.1 (via `packageManager` field) | Package manager                                 |
| Node.js        | 22 or 24 (via `engines.node`)      | Runtime                                         |
| GitHub Actions | n/a                                | CI orchestration via `.github/workflows/ci.yml` |
| Playwright     | `^1.x`                             | E2E runner                                      |
| FOSSA          | n/a                                | License compliance check                        |
...
This section is intentionally sparse; the comprehensive security posture is in §6.4 below.

## 6.4 Security Architecture Detail

The post-refactor security posture is the cumulative result of Workstream C (Authorization, Audit, User Tracking), with backward-compatibility boundaries preserved across Workstreams A, B, and D.

### 6.4.1 Authentication

The portal supports two authentication paths (both augmented by Workstream C):

1. **GitHub OAuth** via `@backstage/plugin-auth-backend-module-github-provider`. The `signInResolver` in `packages/backend/src/authModuleGithubProvider.ts` has been augmented to:

   - Extract email via the priority chain implemented in `selectPrimaryGithubEmail()`: `result.fullProfile.emails?.find(e => e.primary === true)?.value` (canonical primary), then `result.fullProfile.emails?.[0]?.value` (index-0 fallback), then `result.userinfo?.email` (userinfo fallback), then sentinel `${userId}@unknown.invalid` (RFC 2606 reserved; treated as non-Blitzy).
   - Propagate the resulting email into the Backstage-issued token as a custom JWT claim via `ctx.issueToken({ claims: { sub: userEntityRef, ent: [userEntityRef], email: primaryEmail } })`. The policy then decodes the JWT on every authorization check rather than reading a `BackstageIdentityResponse.profile.email` field (see IR-2 in the Implementation Reality Addendum at the top of this document).
   - Synthesize a `correlationId = crypto.randomUUID()` inside the resolver because the `SignInResolver` callback signature is `(info, ctx) => Promise<BackstageSignInResult>` and does NOT expose the Express `request` object (see IR-4).
   - Emit a `user-login` audit event via `coreServices.auditor` with `severityLevel: 'medium'`, `meta: { provider: 'github', username, emailDomain, userEntityRef, correlationId }`, then `.success({ meta: { entityRef, correlationId } })` on success or `.fail({ error, meta: { ..., correlationId } })` on resolver failure.
   - Wrap the resolver body in try/catch to ensure audit emission on both success and failure paths. The `createEvent` call itself runs inside its own try/catch so the resolver re-throws if the auditor service fails (preserves a fail-loud invariant on audit-channel outages).

2. **Guest sign-in** via `packages/app/src/GuestSignInPage.tsx` (frontend) and `guestSignInPageModule` (registered in `App.tsx` features array). The Guest principal is identified by `user?.principal?.type === 'guest'` or by the entity ref `user:default/guest`. At this checkpoint the Guest path is not augmented with a `user-login` audit event; only GitHub sign-ins emit `user-login`. Adding Guest-provider augmentation is tracked in [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md).

The policy MUST NOT trust client-asserted email; it reads only from the JWT `email` claim, and the JWT is cryptographically verified by `DefaultAuthService.authenticate` (via the Backstage user-info service or the auth backend) before reaching the policy. The `jose.decodeJwt(user.credentials.token)` call inside the policy is a non-verifying decode of the already-validated token, not a trust boundary.

### 6.4.2 Authorization

The active `PermissionPolicy` (introduced by Workstream C) is `BlitzyPermissionPolicy` in `plugins/permission-backend-module-blitzy-policy/src/policy.ts`. It is registered via the new backend module at `plugins/permission-backend-module-blitzy-policy/src/module.ts` and wired into `packages/backend/src/index.ts`. The upstream `@backstage/plugin-permission-backend-module-allow-all-policy` (and the in-repo equivalent at `plugins/permission-backend-module-policy-allow-all/`) is no longer registered.

#### Decision Tree

`BlitzyPermissionPolicy.handle(request, user)` returns a `PolicyDecision` per the following decision tree (O(1), side-effect-free):

1. If `request.permission.attributes.action === 'read'` → return `{ result: AuthorizeResult.ALLOW }` regardless of principal. Read actions cover: `catalog.entity.read`, `catalog.location.read`, the read aspect of catalog refresh, search read paths, and TechDocs read.
2. If the user's email ends in `@blitzy.com` (case-insensitive `String.prototype.endsWith` strict-suffix match — does not accept subdomains like `@dev.blitzy.com`) → return `{ result: AuthorizeResult.ALLOW }` for any permission. Email is extracted by `BlitzyPermissionPolicy.extractEmail()`, which first checks `user.info?.email` (forward-compat path — Backstage's default `UserInfoService` populates only `sub` and `ent` from JWT claims at this checkpoint, so this path is a no-op) and otherwise decodes the JWT via `jose.decodeJwt(user.credentials.token).email` (a non-verifying decode of an already-verified token). See IR-2 in the Implementation Reality Addendum.
3. If the principal is Guest (detected via `user?.info?.userEntityRef === 'user:default/guest'` OR `user?.principal?.type === 'guest'`) → return `{ result: AuthorizeResult.DENY }` for write/edit actions.
4. If the user's email domain is anything other than `@blitzy.com` → return `{ result: AuthorizeResult.DENY }` for write/edit actions (create, update, delete, refresh, register, validate).
5. If the user has no email (e.g., GitHub user with private email and no `user:email` scope) → policy treats this as a non-Blitzy principal and returns DENY for write actions (deny-by-default per AAP §0.1.3 "any user logging in with a domain other than `@blitzy.com` or as a Guest must be strictly assigned read-only access").

#### Permission Identifiers Referenced

The new policy references the following permission identifiers from `@backstage/plugin-catalog-common`:

- `catalogEntityReadPermission` (action: read; always ALLOW)
- `catalogEntityCreatePermission` (action: create; ALLOW for `@blitzy.com`, DENY otherwise)
- `catalogEntityDeletePermission` (action: delete; ALLOW for `@blitzy.com`, DENY otherwise)
- `catalogEntityRefreshPermission` (action: update; ALLOW for `@blitzy.com`, DENY otherwise)
- `catalogEntityValidatePermission` (action: read; always ALLOW — read-style validation)
- `catalogLocationCreatePermission`, `catalogLocationDeletePermission` (location mutation; same domain check)

#### Configuration Requirement

`app-config.yaml` MUST contain `permission.enabled: true`. Without this flag, Backstage's permission router does not invoke the policy and all requests are silently allowed. This is documented as a critical operational gotcha in `docs/refactor/onboarding-addendum.md` §8 (Common Pitfalls).

### 6.4.3 Audit

Two audit event types (introduced by Workstream C) are emitted via `coreServices.auditor` (`AuditorService.createEvent(...).success(...) / .fail(...)`):

1. **`user-login`** — Emitted on every GitHub sign-in attempt. (Guest provider augmentation is tracked in `next-tasks.md`; not emitted at this checkpoint.)

   - Source: `packages/backend/src/authModuleGithubProvider.ts` (augmented `signInResolver`).
   - `severityLevel: 'medium'`
   - `meta: { provider: 'github', username, emailDomain, userEntityRef, correlationId }`
   - Emitted via `.success({ meta: { entityRef, correlationId } })` on success and `.fail({ error, meta: { ..., correlationId } })` on resolver failure.
   - **Correlation:** The `correlationId` is a synthetic UUID generated inside the resolver via `crypto.randomUUID()` because the `SignInResolver` callback does not expose the Express `request` object (see IR-4). It is NOT identical to the inbound HTTP request's correlation id; operators correlating back to the request must match surrounding log lines on the `requestId` field emitted by `coreServices.httpRouter`.

2. **`entity-access`** — Emitted on every user-credentialed entity read.
   - Source: `plugins/catalog-backend-module-access-audit/src/module.ts` (new backend module that wraps the catalog query endpoints as Express middleware).
   - `severityLevel: 'low'`
   - `meta: { entityRef, principal, action: 'read' }`
   - Emitted via `.success()` after the catalog returns the entity.
   - **Correlation:** Because the access-audit middleware runs in HTTP request context, it passes the inbound `request` object to `auditor.createEvent({ request, ... })` so the event carries the canonical request-scoped correlation id (NOT a synthetic UUID).

Events land in the backend's structured JSON log on stdout and (if configured) in the AuditorService's auxiliary log channel. The audit-event channel provides the operator's narrative trail for sign-in / entity-access signals; complementary Prometheus counters covering the same signals are implemented and emitting (see §6.4.4 below and IR-5), so operators can use either signal — or both — for monitoring and alerting.

### 6.4.4 Observability

OpenTelemetry traces span the permission policy `.handle()` invocations and the audit `.createEvent(...)` calls via existing auto-instrumentation in `packages/backend/src/instrumentation.js`. No new SDK changes are introduced.

#### Custom Prometheus Counters (Implemented and Emitting)

> **Implementation status (IR-5):** All three counters below are emitted by the source code at this checkpoint and verified at runtime via `curl http://localhost:9464/metrics`. The implementation imports `{ Counter, metrics } from '@opentelemetry/api'` (the metrics API was unified into `@opentelemetry/api` in newer OpenTelemetry SDK versions; the obsolete `@opentelemetry/api-metrics` package referenced in earlier text is not used). The Grafana dashboard (`docs/observability/dashboard-template.json`) and the operator runbooks (`docs/observability/dashboards.md`) render real data on a running backend.

| Counter                             | Labels                                                                                                                                                                                                    | Emission site                                                                                                                                                                                                                   |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `blitzy_permission_decisions_total` | `result` ∈ {ALLOW, DENY}, `email_domain` (bucketed: `blitzy.com`, `other`, `guest`), `action` ∈ {read, create, update, delete, refresh, register, validate}, `otel_scope_name="blitzy-permission-policy"` | Declared in `plugins/permission-backend-module-blitzy-policy/src/metrics.ts`; incremented inside `BlitzyPermissionPolicy.handle()` in `plugins/permission-backend-module-blitzy-policy/src/policy.ts` on every decision.        |
| `user_login_total`                  | `provider` ∈ {github, guest}, `email_domain` (bucketed: `blitzy.com`, `other`, `guest`, `unknown`), `otel_scope_name="blitzy-backstage-backend"`                                                          | Declared in `packages/backend/src/metrics.ts`; incremented inside the augmented `signInResolver` at `packages/backend/src/authModuleGithubProvider.ts` on every successful sign-in.                                             |
| `entity_access_total`               | `action` ∈ {read}, `otel_scope_name="blitzy-catalog-access-audit"`                                                                                                                                        | Declared in `plugins/catalog-backend-module-access-audit/src/metrics.ts`; incremented inside the access-audit middleware at `plugins/catalog-backend-module-access-audit/src/module.ts` on every user-credentialed entity read. |

Auto-instrumented metrics emitted today (used by Grafana panels 3–6 in the template) include `http_server_duration_bucket/_count/_sum` (in ms; labels `http_route`, `http_status_code`, `http_method`, `http_scheme`, `net_host_name`, `net_host_port`), `http_client_duration_*`, `v8js_memory_heap_used`, `v8js_memory_heap_limit`, `nodejs_eventloop_delay_*`, `v8js_gc_duration_*`, and Prometheus `up{}` scrape liveness. These cover the catalog query latency panel, HTTP error rate panel, Node.js heap panel, and service health panel of the dashboard template. The legacy names `http_server_request_duration_seconds`, `http_server_requests_total`, `process_resident_memory_bytes`, `nodejs_heap_size_used_bytes`, and `nodejs_eventloop_lag_seconds` are NOT emitted by `@opentelemetry/auto-instrumentations-node` v0.67.0; any earlier docs that referenced them have been corrected in `docs/observability/dashboards.md`, `docs/observability/dashboard-template.json`, and the companion Project Guide.

#### Cardinality Discipline

To avoid Prometheus cardinality explosion under the implemented counters:

- `email_domain` is bucketed into a small fixed set (`blitzy.com`, `other`, `guest`, `unknown` for sign-in; `blitzy.com`, `other`, `guest` for permission decisions) via the `bucketEmailDomain()` / `bucketSignInEmailDomain()` helpers; the full email address is NEVER a label.
- `entityRef` is NOT a label on any counter; it appears only in the `meta` of the corresponding audit event.
- User IPs and session IDs are NOT labels.

#### Dashboard

The Grafana dashboard template at `docs/observability/dashboard-template.json` defines six panels:

1. **Audit Events Per Minute** (stacked: `user_login_total` + `entity_access_total`) — pending custom counter implementation; renders "No data" at this checkpoint.
2. **Permission Decisions (ALLOW / DENY)** (with red/green color overrides) — pending custom counter implementation; renders "No data" at this checkpoint.
3. **Catalog query latency p50/p95/p99** — available today (auto-instrumented).
4. **HTTP error rate** — available today (auto-instrumented).
5. **Node.js heap usage** — available today (auto-instrumented).
6. **Service Health** — available today (Prometheus `up{}` scrape liveness).
7. Service health (single-stat `up{job="blitzy-backstage"}`)

Operator-facing documentation at `docs/observability/dashboards.md` describes:

- Structured logging configuration and correlation ID propagation
- OpenTelemetry trace export (OTLP exporter configuration)
- Prometheus metrics catalog (the three counters above)
- Health / readiness endpoint URLs
- Grafana dashboard import procedure

### 6.4.5 Secrets, PII, and Sensitive Data

- Audit log meta MUST NEVER contain the full OAuth access token, refresh token, or user identifying information beyond the entity ref and bucketed email domain.
- The `/metrics` endpoint MUST be bound to `localhost` (or behind a service mesh policy) in production deployments.
- The `email_domain` label on Prometheus counters is bucketed (see §6.4.4 above), preventing leakage of individual user emails through the metrics endpoint.
- The JWT `email` claim (see IR-2) is server-issued and cryptographically signed by `DefaultAuthService` before being returned to the client. Client tampering is not possible because the resolver runs server-side and the JWT signature is verified upstream of the policy by `DefaultAuthService.authenticate`. The policy's `jose.decodeJwt` call is a non-verifying decode of an already-validated token.

### 6.4.6 Backward Compatibility

- The catalog data model (`@backstage/catalog-model` `System`, `Owner`, `Group`, `Component` relations) remains intact. Only UI surfaces that display these relations are removed; the backend data model and catalog API responses are unchanged. Out-of-band consumers (catalog REST/GraphQL clients, automation scripts) continue to function.
- Per-entity TechDocs functionality is preserved; only the global `/docs` index route is removed.
- The Settings page route (`/settings`) is unchanged; only its mount point moves from the sidebar into the top-right cluster.
- Audit consumers using the standard `AuditorService` channel receive the new event types without configuration change.
- No `package.json` workspace dependencies are removed; the `@backstage/plugin-home` family remains because per-entity extensions may register against it.

---

## See also

- [`./Project Guide.md`](./Project%20Guide.md) — Operational project guide (executive summary, hours breakdown, test results, runtime validation, compliance review, risk assessment, development guide, appendices)
- [`../../docs/refactor/decision-log.md`](../../docs/refactor/decision-log.md) — Decision log (per R3 Explainability)
- [`../../docs/refactor/traceability-matrix.md`](../../docs/refactor/traceability-matrix.md) — Bidirectional requirement ↔ file/test mapping (per R3)
- [`../../docs/refactor/architecture-before-after.md`](../../docs/refactor/architecture-before-after.md) — Mermaid before/after diagrams (per R4)
- [`../../docs/refactor/onboarding-addendum.md`](../../docs/refactor/onboarding-addendum.md) — Clean-machine setup, LocalGCP, customization (per R2)
- [`../../docs/refactor/next-tasks.md`](../../docs/refactor/next-tasks.md) — Discovered improvements out of scope (per R2)
- [`../../docs/observability/dashboards.md`](../../docs/observability/dashboards.md) — Observability operator docs (per R1)
- [`../../docs/observability/dashboard-template.json`](../../docs/observability/dashboard-template.json) — Grafana dashboard JSON (per R1)
- [`../../blitzy-deck/executive-summary.html`](../../blitzy-deck/executive-summary.html) — Single self-contained reveal.js executive presentation (per R5)
- [`../../docs/auth/github/provider.md`](../../docs/auth/github/provider.md) — GitHub auth provider with audit emission
- [`../../docs/auth/identity-resolver.md`](../../docs/auth/identity-resolver.md) — Augmented `signInResolver`
